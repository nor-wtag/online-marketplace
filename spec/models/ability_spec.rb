...
RSpec.describe Ability do
  ...
  let(:another_buyer_review) { create(:review, buyer: another_buyer, product: product) }
  ...
  context "when the user is a seller" do
    ...
    it { is_expected.to be_able_to(:read, Order, order_items: { product: { seller_id: seller.id } }) }
    it { is_expected.not_to be_able_to(:update, Order, order_items: { product: { seller_id: buyer.id } }) }

    it { is_expected.to be_able_to(:read, OrderItem, order: { order_items: { product: { seller_id: seller.id } } }) }
    it { is_expected.to be_able_to(:update_status, order_item) if order_item.product.seller == seller }
    it { is_expected.not_to be_able_to(:update, order_item) if order_item.product.seller != seller }
  ...
  context "when the user is a buyer" do
    ...
    it { is_expected.to be_able_to(:delete, review) }
    it { is_expected.not_to be_able_to(:update, another_buyer_review) }
    it { is_expected.not_to be_able_to(:destroy, another_buyer_review) }
    ...
    it { is_expected.to be_able_to(:create, CartItem, cart: { buyer_id: buyer.id }) }
    it { is_expected.to be_able_to(:read, CartItem, cart: { buyer_id: buyer.id }) }
    it { is_expected.to be_able_to(:update, CartItem, cart: { buyer_id: buyer.id }) }
    it { is_expected.to be_able_to(:destroy, CartItem, cart: { buyer_id: buyer.id }) }

    it { is_expected.to be_able_to(:create, Order) }
    it { is_expected.to be_able_to(:read, Order, buyer_id: buyer.id) }
    it { is_expected.to be_able_to(:update, Order, buyer_id: buyer.id) }
    it { is_expected.to be_able_to(:cancel, Order, buyer_id: buyer.id) }

    it { is_expected.to be_able_to(:create, OrderItem) }
    it { is_expected.to be_able_to(:read, OrderItem, order: { buyer_id: buyer.id }) }
    it { is_expected.to be_able_to(:update, OrderItem, order: { buyer_id: buyer.id }) }
    it { is_expected.to be_able_to(:destroy, OrderItem, order: { buyer_id: buyer.id }) }
    it { is_expected.to be_able_to(:update_status, OrderItem.new(order: order, status: 'delivered'), status: 'delivered') if order.buyer == buyer }
